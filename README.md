...
- **API Gateway WebSocket API**: 별도의 장기 실행 서버 없이 브라우저와 **연결 단위 푸시**를 붙일 수 있습니다.
- **프리 티어**: Lambda·DynamoDB·API Gateway 무료 한도 안에서 과제 규모를 운영하기에 부담이 적었습니다.
- **리전 `ap-northeast-2` (서울)**: 데모·검증 시 지연을 줄이기 위해 사용했습니다.
...
과제 기간 안에 **기능과 배포 재현성**에 집중하고 싶어, VM·컨테이너 오케스트레이션은 최소화하는 방향을 택했습니다.
...
- **규칙은 한곳에서**: `stateMachine.ts`의 상태 전이 규칙을 Express와 Lambda가 공유해, “로컬에서는 되는데 배포만 다름”을 줄이려 했습니다.
- **배포 이슈 시 대안**: 필요하면 Express 기반으로 다른 호스팅으로 옮기는 것도 가능한 구조입니다.

---

## 4. 모듈 구성 이유

**어디를 보면 되는지** 빨리 찾을 수 있도록, 역할별로 폴더를 나눴습니다.
